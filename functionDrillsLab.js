...
let sum = add(25, 13215465);

console.log(`The variable sum is ${sum}.`);
console.log("Here is the add function with one and two - " + add(1,2));
...
let nameGreeting = nameCheck("John Jacob Jingleheimerschmidt");
...
console.log(faveColorFinder("torquoise"));
...
let oddChecker = thatsOdd(21634568);
...
function theEliminator(arr, ded){
  for (i = 0; i < arr.length; i++){
    if (arr[i] === ded){
      arr.splice(i, 1);
    }
...
function trolley(gold){
  let frog = 3;
  let amount = Math.floor(gold/frog);
  let remainder = gold%frog;
  if (amount === 1){
    return `You can buy ${amount} frog with ${remainder} gold left over.`;
  }else {
    return `You can buy ${amount} frogs with ${remainder} gold left over.`;
  }  
}

let totalFrogs = trolley(15);
...
function trolley2(){
  return "I already did it without you asking :p";
}

let totalFrogs2 = trolley2();
...
let arrayIsAscending = asc(sampleArray);
